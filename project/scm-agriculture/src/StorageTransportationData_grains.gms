...
* The report on transportation sector in India shows that the working hours for truck operators range from 8-14 hours. So 10 hours is a reasonable assumption on a average
  ...
  DieselFuelCost "The cost of diesel in $ per gallon used to run the machines which is used to calculate the fuel consumption cost"
                   /2.20/
  ;
...
* This parameter is used to model Type I energy cane scenario with high sugar content.
  BiomassStorageTimeLimit "The number of days after harvesting within which the harvested biomass must be sent to the biorefinery for processing (number of days)" 
                          /2/
  ;
...
* Taken from Kumar and Sokhansanj 2007 (IBSAL paper) (value reported for round bale scenario which considers a front end bale loader)
  StorageStackingCost "Cost of stacking (which includes unloading from the short range bale wagon and stacking at the storage facility) a bale or loaf (at either storage locations) in $ per dry tonne"
                       /1.19/
...
* Taken from Kumar and Sokhansanj 2007 (IBSAL paper)
  GroundBiomassStorageLoadingCost "Cost of loading ground biomass onto truck (for ensiling operation) for long range transportation in $ per dry tonne"
                                     /3.62/
* Taken from Kumar and Sokhansanj 2007 (IBSAL paper)
  GroundBiomassRefineryUnloadingCost "Cost of unloading ground biomass onto truck (for ensiling operation) for long range transportation in $ per dry tonne"
                                      /0.93/
  ;
...
* Assumed due to lack of data in literature
  MaximumTransportationFleetSize "Total number of trucks available for all farms for biomass transportation on a day" 
...
* Check the calculations excel sheet.
  FCITransportationDryMatterLossRate "Fraction of dry matter lost during transportation related to FCI operations" 
                                      /0.003267513/
