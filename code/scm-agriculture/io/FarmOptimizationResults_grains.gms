...
$Ontext
    The Calling file should declare the following parameter and set.
    SET:
...
*
* Calculating the desired parameters after the farm management optimization problems and storing 
* the results in appropriate parameters
*
Par_CumulativeFarmGateGrainAvailability(HarvestingHorizonAggregation) = sum((FarmNumber),
    Par_HarvestToFarmGate(HarvestingHorizonAggregation,FarmNumber));

Par_CumulativeFarmGateGrainAvailability(NonHarvestingHorizonAggregation) = sum((FarmNumber),
...
*=================================================================================================
* Transportation operation related modifications: Based on the output form of Grain from a 
* given farm, the type of transportation equipment that can be used is limited. A correlation
* indicator is created here which enables us to limit the number of transportation equipment
* considered for selection. This will reduce the number of possibilities
